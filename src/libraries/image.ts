...
const PC_THUMBNAIL_MAX_WIDTH = 384;
const PC_THUMBNAIL_DEFAULT_HEIGHT = 216;
...
export function getImageSize(imagePath: string, type: 'thumbnail' | 'article') {
  ...
  switch(type) {
    case 'thumbnail':
      const pcThumbnailRatio = PC_THUMBNAIL_MAX_WIDTH / (image?.width ?? PC_THUMBNAIL_MAX_WIDTH);
      const spThumbnailRatio = SP_THUMBNAIL_MAX_WIDTH / (image?.width ?? SP_THUMBNAIL_MAX_WIDTH);
      return {
        pc: {
          width: PC_THUMBNAIL_MAX_WIDTH,
          height: (image?.height ?? PC_THUMBNAIL_DEFAULT_HEIGHT) * pcThumbnailRatio,
        },
        sp: {
          width: SP_THUMBNAIL_MAX_WIDTH,
          height: (image?.height ?? SP_THUMBNAIL_DEFAULT_HEIGHT) * spThumbnailRatio,
        },
      };
    case 'article':
      const pcArticleRatio = PC_ARTICLE_MAX_WIDTH / (image?.width ?? PC_ARTICLE_MAX_WIDTH);
      const spArticleRatio = SP_ARTICLE_MAX_WIDTH / (image?.width ?? SP_ARTICLE_MAX_WIDTH);
      return {
        pc: {
